...
| Convites preparados | `memberInvitations`, `createMemberInvitation`, Central de Divulgação | B | Prepare convites e registre o canal previsto quando a funcionalidade estiver exposta por área operacional ativa. O sistema não envia e-mails, WhatsApp ou mensagens externas automaticamente. |
| Comunicação assistida | Central de Divulgação, contatos consentidos | B | Estruture contatos e histórico para apoio manual. Não chamar de automação autônoma. |
...
| Personalização por pedido aprovado | `applicationAccessTokens`, `ApplicationPersonalization.tsx`, `/personalizar?codigo=...` | A | Após aprovação do pagamento, o comprador usa o token individual do pedido para configurar dados públicos iniciais, senha e acesso ao Escritório Virtual. |
