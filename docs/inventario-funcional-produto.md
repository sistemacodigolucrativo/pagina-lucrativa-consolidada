...
| Dashboard do Escritório Virtual | `getMemberOverview`, `MemberOffice.tsx` | A | Centralize visão geral, campanhas, produtos, saldo, tickets e movimentações da sua operação. |
...
| Extrato e ganhos | `transactions`, `MemberEarnings.tsx`, `getMemberFinance` | B | Acompanhe saldo confirmado, ganhos, pendências e lançamentos. Registros enviados pelo membro entram em análise e o módulo não é um processador de pagamentos. |
| Registro de venda ou saque | `member.createFinanceEntry`, `MemberEarnings.tsx` | B | Informe uma venda ou solicitação de saque para análise administrativa. Não prometer aprovação, pagamento ou saque automático. |
