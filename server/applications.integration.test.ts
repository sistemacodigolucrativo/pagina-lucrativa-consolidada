...
describe("gestão de solicitações públicas", () => {
  ...
  it("organiza a página de pagamento em jornada linear sem alterar comprovante PIX", async () => {
    ...
    expect(paymentPage).toContain("showPaymentMethodChooser");
    expect(paymentPage).toContain("paymentDetailsStep");
    expect(paymentPage).toContain("Pague com PIX");
    expect(paymentPage).toContain("Pague pelo checkout");
    ...
    expect(paymentPage).toContain("{buyerDetailsContent}</article>\n\n          <article className=\"min-w-0 rounded-2xl border border-emerald-300/20");
    expect(paymentPage).not.toContain("lg:sticky");
    ...
    expect(paymentPage).toContain("Este é o seu apresentador");
    expect(paymentPage).toContain("showPaymentStatusSummary");
