...
export const appRouter = router({
  ...
  admin: router({
    ...
    createTransaction: adminProcedure.input(z.object({ userId: z.number().int().positive(), campaignId: z.number().int().positive().nullable().optional(), type: z.enum(["sale", "commission", "adjustment", "withdrawal"]), description: z.string().trim().min(3).max(320), amountCents: positiveCentsZodSchema, status: z.enum(["pending", "posted", "void"]) })).mutation(({ ctx, input }) => createAdminTransaction(ctx.user.id, input)),
