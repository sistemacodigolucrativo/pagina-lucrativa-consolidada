...
describe("anotações administrativas — publicações, e-books e agradecimentos", () => {
  it("mantém Publicações responsiva e remove configurações internas da listagem", () => {
    ...
    expect(source).toContain("grid w-full grid-cols-2 gap-2 sm:w-auto sm:flex");
  });

  it("organiza Biblioteca de e-books como introdução, regras, lista e formulário sob demanda", () => {
    const source = read("client/src/pages/AdminEbooks.tsx");
    expect(source).toContain("const [formOpen, setFormOpen]");
    ...
    expect(source).toContain('isLibraryAdmin ? "grid gap-5" : "grid gap-5 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.35fr)] lg:items-start"');
    expect(source).toContain('isLibraryAdmin ? "contents" : "order-1 space-y-5 lg:order-2"');
    expect(source).toContain('isLibraryAdmin ? "order-1 rounded-2xl');
  });

  it("faz os cards de agradecimentos filtrarem a lista pelo status real", () => {
    const source = read("client/src/pages/AdminTestimonials.tsx");
    expect(source).toContain('onClick={() => setFilter(status)}');
    expect(source).toContain('if (filter !== "all" && item.status !== filter) return false;');
    ...
    expect(source).toContain('aria-pressed={filter === status}');
    expect(source).toContain('aria-controls="testimonial-moderation-list"');
  });
});
