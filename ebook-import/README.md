...
O fluxo oficial de banco é `scripts/sync-packaged-content.mjs`. Ele roda em modo seguro por padrão (`--dry-run`) e só escreve no banco com `--apply`. O script usa `ebook-manifest.tsv`, `shared/ebookLibraryCatalog.ts`, `fontes_importados/` e `content-seeds/academy-courses.json` para recriar o conteúdo padrão em uma VPS nova sem duplicar registros.

O manifesto da Academia usa 29 e-books deste diretório como materiais de curso, organizados em 11 cursos e 16 módulos. Esses registros ficam com `usage: "both"` para que o mesmo PDF continue na Biblioteca e também apareça na Academia. Os demais PDFs continuam como materiais independentes da Biblioteca.

O manifesto aceita metadados opcionais de Academia/Biblioteca, incluindo `usage`, `libraryCategory`, `courseTitle`, `courseOrder`, `moduleTitle`, `moduleOrder`, `lessonOrder` e `level`, sem exigir migration de banco. Para a estrutura oficial de cursos, prefira `content-seeds/academy-courses.json`; o TSV permanece como fonte dos e-books e das categorias. O script legado `scripts/import-ebooks.mjs` permanece apenas por compatibilidade operacional; para bootstrap novo ou ressincronização use `sync-packaged-content.mjs`.
