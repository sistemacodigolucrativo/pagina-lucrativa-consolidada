...
O manifesto aceita metadados opcionais de Academia/Biblioteca, incluindo `usage`, `libraryCategory`, `courseTitle`, `courseOrder`, `moduleTitle`, `moduleOrder`, `lessonOrder` e `level`, sem exigir migration de banco. O script legado `scripts/import-ebooks.mjs` permanece apenas por compatibilidade operacional; para bootstrap novo ou ressincronização use `sync-packaged-content.mjs`.
