...
export default function AdminPublications() {
  ...
  return <DashboardLayout menuItems={adminMenu} title="Administração"><main className="mx-auto w-full max-w-7xl space-y-7 p-4 sm:p-6 lg:p-8">
    ...
    <section className="grid min-w-0 gap-6 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
      ...
      <section className="min-w-0 rounded-2xl border border-white/10 bg-zinc-950/60 p-4 sm:p-5"><div className="mb-4 flex flex-col items-start gap-1 sm:flex-row sm:items-center sm:justify-between"><h2 className="font-medium text-white">Conteúdos cadastrados</h2><span className="text-xs uppercase tracking-wider text-zinc-500">{managedItems.length} itens</span></div>{content.isLoading ? <p className="text-sm text-zinc-400">Carregando...</p> : managedItems.length ? <div className="space-y-3">{managedItems.map(item => <article key={item.id} className="rounded-xl border border-white/10 bg-black/25 p-4"><div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0 flex-1"><span className="text-xs uppercase tracking-wider text-emerald-200">{statusLabel(item.status)}</span><h3 className="mt-1 font-medium text-white [overflow-wrap:anywhere]">{item.title}</h3><p className="mt-1 line-clamp-2 text-sm text-zinc-400 [overflow-wrap:anywhere]">{item.summary || item.body || "Sem descrição."}</p></div><div className="grid w-full grid-cols-2 gap-2 sm:w-auto sm:flex"><button type="button" onClick={() => edit(item)} className="inline-flex w-full items-center justify-center gap-1 rounded-lg border border-white/15 px-3 py-2 text-sm text-zinc-200 sm:w-auto"><PencilLine className="size-4" />Editar</button><button type="button" disabled={deletingId === item.id} onClick={() => void removeItem(item)} className="inline-flex w-full items-center justify-center gap-1 rounded-lg border border-red-400/25 px-3 py-2 text-sm text-red-200 disabled:opacity-60 sm:w-auto"><Trash2 className="size-4" />{deletingId === item.id ? "Excluindo..." : "Excluir"}</button></div></div></article>)}</div> : <p className="rounded-xl border border-dashed border-white/15 p-5 text-sm text-zinc-400">Nenhum conteúdo registrado ainda.</p>}</section>
    </section>
