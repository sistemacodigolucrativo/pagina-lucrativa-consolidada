...
export default function AdminPublications() {
  ...
  useEffect(() => {
    ...
    setEditingId(null);
    setForm(blankForm(config.kind));
  }, [config.kind, config.splitFlow, content.data, editRouteId, isCreateScreen]);
  ...
  const listPanel = (
    <section className="min-w-0 overflow-hidden rounded-2xl border border-white/10 bg-zinc-950/60 p-4 sm:p-5">
      <div className="mb-4 flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0"><h2 className="break-words font-medium text-white">Conteúdos cadastrados</h2><span className="mt-1 block text-xs uppercase tracking-wider text-zinc-500">{managedItems.length} itens</span></div>
        {config.splitFlow ? <button type="button" onClick={openCreate} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-emerald-300 px-4 py-2 text-sm font-semibold text-black transition active:scale-[.98] sm:w-auto"><PlusCircle className="size-4" />Criar</button> : null}
      </div>
      {content.isLoading ? <p className="text-sm text-zinc-400">Carregando...</p> : managedItems.length ? <div className="space-y-3">{managedItems.map(item => <article key={item.id} role={config.splitFlow ? "button" : undefined} tabIndex={config.splitFlow ? 0 : undefined} onClick={config.splitFlow ? () => edit(item) : undefined} onKeyDown={config.splitFlow ? event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); edit(item); } } : undefined} className={`min-w-0 overflow-hidden rounded-xl border border-white/10 bg-black/25 p-3 sm:p-4 ${config.splitFlow ? "cursor-pointer transition hover:border-emerald-300/35 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300/60" : ""}`}><div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0 flex-1"><span className="text-xs uppercase tracking-wider text-emerald-200">{statusLabel(item.status)}</span><h3 className="mt-1 font-medium text-white [overflow-wrap:anywhere]">{item.title}</h3><p className="mt-1 line-clamp-2 text-sm text-zinc-400 [overflow-wrap:anywhere]">{item.summary || item.body || "Sem descrição."}</p></div><div className="grid w-full min-w-0 grid-cols-1 gap-2 min-[420px]:grid-cols-2 sm:w-auto sm:flex"><button type="button" onClick={event => { event.stopPropagation(); edit(item); }} className="inline-flex min-h-10 w-full items-center justify-center gap-1 rounded-lg border border-white/15 px-3 py-2 text-sm text-zinc-200 sm:w-auto"><PencilLine className="size-4" />Editar</button><button type="button" disabled={deletingId === item.id} onClick={event => { event.stopPropagation(); void removeItem(item); }} className="inline-flex min-h-10 w-full items-center justify-center gap-1 rounded-lg border border-red-400/25 px-3 py-2 text-sm text-red-200 disabled:opacity-60 sm:w-auto"><Trash2 className="size-4" />{deletingId === item.id ? "Excluindo..." : "Excluir"}</button></div></div></article>)}</div> : <p className="rounded-xl border border-dashed border-white/15 p-5 text-sm text-zinc-400">Nenhum conteúdo registrado ainda.</p>}
    </section>
  );
