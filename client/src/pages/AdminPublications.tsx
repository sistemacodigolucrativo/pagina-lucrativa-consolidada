...
export default function AdminPublications() {
  ...
  return <DashboardLayout menuItems={adminMenu} title="Administração"><main className="mx-auto w-full min-w-0 max-w-7xl space-y-7 overflow-x-clip p-4 sm:p-6 lg:p-8">
    <header className="min-w-0 space-y-2"><span className="text-xs uppercase tracking-[0.16em] text-emerald-300">{config.eyebrow}</span><h1 className="break-words text-2xl font-semibold text-white sm:text-3xl">{config.title}</h1><p className="max-w-3xl text-sm leading-6 text-zinc-300">{config.description}</p></header>
    <section className="grid min-w-0 gap-6 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
      <form onSubmit={submit} className="min-w-0 space-y-4 overflow-hidden rounded-2xl border border-white/10 bg-zinc-950/60 p-4 sm:p-5">
        ...
        {config.kind === "material" ? <section className="min-w-0 space-y-4 rounded-xl border border-emerald-300/20 bg-emerald-300/5 p-3 sm:p-4"><p className="text-sm leading-6 text-emerald-50">O arquivo precisa estar compartilhado no Google Drive com permissão adequada para os membros.</p><div className="grid min-w-0 gap-4 sm:grid-cols-2"><label className="min-w-0 text-sm text-zinc-200">Categoria do recurso<select value={form.resourceCategory} onChange={event => setForm({ ...form, resourceCategory: event.target.value })} className="mt-1 w-full rounded-lg border border-white/15 bg-black px-3 py-2 text-white">{resourceCategories.map(category => <option key={category}>{category}</option>)}</select></label><label className="min-w-0 text-sm text-zinc-200">Tipo do recurso<select value={form.resourceType} onChange={event => setForm({ ...form, resourceType: event.target.value })} className="mt-1 w-full rounded-lg border border-white/15 bg-black px-3 py-2 text-white">{resourceTypes.map(type => <option key={type}>{type}</option>)}</select></label></div><label className="block text-sm text-zinc-200">Link do Google Drive<input type="url" value={form.resourceUrl} onChange={event => setForm({ ...form, resourceUrl: event.target.value })} required={form.status === "published"} className="mt-1 w-full rounded-lg border border-white/15 bg-black px-3 py-2 text-white" placeholder="https://drive.google.com/..." /></label>{formError ? <p role="alert" className="rounded-lg border border-red-400/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">{formError}</p> : null}</section> : formError ? <p role="alert" className="rounded-lg border border-red-400/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">{formError}</p> : null}
        <div className="grid w-full grid-cols-1 gap-2 sm:flex sm:w-auto sm:flex-wrap sm:gap-3"><button disabled={busy} className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-emerald-300 px-4 py-2 text-sm font-semibold text-black disabled:opacity-60 sm:w-auto"><PlusCircle className="size-4" />{create.isPending || update.isPending ? "Salvando..." : editingId ? "Salvar alterações" : "Criar"}</button>{editingId ? <button type="button" onClick={reset} className="w-full rounded-lg border border-white/15 px-4 py-2 text-sm text-zinc-200 sm:w-auto">Cancelar</button> : null}</div>
      </form>
      <section className="min-w-0 overflow-hidden rounded-2xl border border-white/10 bg-zinc-950/60 p-4 sm:p-5"><div className="mb-4 flex flex-col items-start gap-1 sm:flex-row sm:items-center sm:justify-between"><h2 className="font-medium text-white">Conteúdos cadastrados</h2><span className="text-xs uppercase tracking-wider text-zinc-500">{managedItems.length} itens</span></div>{content.isLoading ? <p className="text-sm text-zinc-400">Carregando...</p> : managedItems.length ? <div className="space-y-3">{managedItems.map(item => <article key={item.id} className="min-w-0 overflow-hidden rounded-xl border border-white/10 bg-black/25 p-3 sm:p-4"><div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0 flex-1"><span className="text-xs uppercase tracking-wider text-emerald-200">{statusLabel(item.status)}</span><h3 className="mt-1 font-medium text-white [overflow-wrap:anywhere]">{item.title}</h3><p className="mt-1 line-clamp-2 text-sm text-zinc-400 [overflow-wrap:anywhere]">{item.summary || item.body || "Sem descrição."}</p></div><div className="grid w-full min-w-0 grid-cols-1 gap-2 min-[420px]:grid-cols-2 sm:w-auto sm:flex"><button type="button" onClick={() => edit(item)} className="inline-flex w-full items-center justify-center gap-1 rounded-lg border border-white/15 px-3 py-2 text-sm text-zinc-200 sm:w-auto"><PencilLine className="size-4" />Editar</button><button type="button" disabled={deletingId === item.id} onClick={() => void removeItem(item)} className="inline-flex w-full items-center justify-center gap-1 rounded-lg border border-red-400/25 px-3 py-2 text-sm text-red-200 disabled:opacity-60 sm:w-auto"><Trash2 className="size-4" />{deletingId === item.id ? "Excluindo..." : "Excluir"}</button></div></div></article>)}</div> : <p className="rounded-xl border border-dashed border-white/15 p-5 text-sm text-zinc-400">Nenhum conteúdo registrado ainda.</p>}</section>
    </section>
