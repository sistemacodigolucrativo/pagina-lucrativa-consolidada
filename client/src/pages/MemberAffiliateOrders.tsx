      ...
      <section className="flex max-h-[calc(100dvh-1rem)] w-full max-w-lg flex-col overflow-hidden rounded-2xl border border-emerald-300/25 bg-zinc-950 shadow-2xl sm:max-h-[calc(100dvh-2rem)]">
        ...
        <div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-3 sm:p-4">
          <section className="rounded-2xl border border-white/10 bg-black/25 p-2.5 sm:p-3">
            <div className="flex items-center justify-between gap-3"><h3 className="font-medium text-white">Comprovante</h3>{selectedReceipt?.contentType.startsWith("image/") ? <span className="text-[11px] text-zinc-500">Toque para ampliar</span> : null}</div>
            {selectedReceipt ? <div className="mt-2 space-y-2">
              {selectedReceipt.contentType.startsWith("image/") ? <button type="button" onClick={() => { setViewerOpen(true); setImageZoom(1); }} className="group mx-auto flex size-28 items-center justify-center overflow-hidden rounded-xl border border-white/10 bg-black/35 p-1.5 transition hover:border-emerald-300/40 sm:size-32" aria-label="Ampliar comprovante">
                <img src={selectedReceipt.fileUrl} alt={selectedReceipt.originalName || "Comprovante enviado"} className="max-h-full max-w-full rounded-lg object-contain shadow-lg transition group-hover:scale-[1.04]" />
