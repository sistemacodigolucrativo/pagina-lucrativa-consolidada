...
import { ArrowLeft, CheckCircle2, Clipboard, CreditCard, Loader2, QrCode, UploadCloud, UserRound } from "lucide-react";
    ...
    <section className="mx-auto w-full max-w-5xl space-y-6">
      ...
      <section className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
        <div className="space-y-6">
          ...
          <article className="rounded-2xl border border-emerald-300/20 bg-emerald-300/5 p-5 sm:p-6">
            ...
            {pixKey ? <div className="mt-4 space-y-4">
            ...
            </div> : <p className="mt-4 text-sm leading-6 text-zinc-400">O responsável ainda não configurou uma chave PIX pública para este pedido.</p>}
          </article>
          ...
          <article className="rounded-2xl border border-white/10 bg-zinc-950/70 p-5 sm:p-6">
            ...
            {paymentLinks.length ? <div className="mt-4 grid gap-3 sm:grid-cols-2">{paymentLinks.map(link => <a key={link.id} href={link.paymentUrl} target="_blank" rel="noreferrer" className="inline-flex min-h-12 items-center justify-center rounded-lg bg-emerald-300 px-4 py-3 text-center text-sm font-semibold text-black transition hover:bg-emerald-200">Pagar com {link.label}</a>)}</div> : <p className="mt-4 text-sm leading-6 text-zinc-400">Nenhum link de checkout externo está habilitado para este responsável.</p>}
          </article>
        ...
        <aside className="space-y-4 rounded-2xl border border-white/10 bg-zinc-950/70 p-5 sm:p-6 lg:sticky lg:top-6 lg:self-start">
          ...
          {receipts.some(receipt => receipt.status === "pending") ? <div className="rounded-xl border border-emerald-300/30 bg-emerald-300/10 p-3 text-sm leading-6 text-emerald-50"><CheckCircle2 className="mb-2 size-5" />Comprovante enviado. O responsável pela Página Lucrativa recebeu sua solicitação.</div> : null}
        </aside>
      </section>
    </section>
  </main>;
