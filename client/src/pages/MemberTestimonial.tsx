  ...
  return <DashboardLayout menuItems={menu} title="Escritório Virtual">
    <main className="mx-auto w-full max-w-6xl space-y-7 p-5 sm:p-8">
      <header className="space-y-2">
        <span className="text-xs uppercase tracking-[0.16em] text-emerald-300">Relato autêntico</span>
        <h1 className="text-3xl font-semibold text-white">Compartilhe sua experiência</h1>
        <p className="max-w-3xl text-sm leading-6 text-zinc-300">Envie somente um relato próprio e verdadeiro sobre sua experiência. O conteúdo fica privado até a revisão da administração e não é publicado automaticamente.</p>
      </header>
      <section className="grid min-w-0 gap-6 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,.95fr)]">
        <form onSubmit={submit} className="order-1 min-w-0 space-y-4 rounded-2xl border border-emerald-300/25 bg-zinc-950/60 p-5 sm:p-6">
          <div className="flex items-center gap-2 text-white"><MessageCircleMore className="size-5 text-emerald-300" /><h2 className="font-medium">Escrever relato</h2></div>
        ...
        <section className="order-2 min-w-0 space-y-4 rounded-2xl border border-white/10 bg-zinc-950/60 p-5 sm:p-6">
          ...
          <p className="text-sm leading-6 text-zinc-300">A administração pode aprovar, solicitar ajuste, arquivar ou manter seu relato em análise. Aprovação significa apenas que o conteúdo está apto para a próxima etapa; não cria uma publicação pública automática.</p>
      ...
      <section className="rounded-2xl border border-white/10 bg-zinc-950/60 p-5 sm:p-6">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><span className="text-xs uppercase tracking-[0.16em] text-emerald-300">Acompanhamento</span><h2 className="mt-1 text-lg font-medium text-white">Meus relatos enviados</h2></div><span className="text-xs uppercase tracking-wider text-zinc-500">{testimonials.data?.length ?? 0} registros</span></div>
        {testimonials.isLoading ? <p className="text-sm text-zinc-400">Carregando seus relatos...</p> : testimonials.isError ? <p className="rounded-xl border border-red-300/30 p-4 text-sm text-red-200">Não foi possível carregar seus relatos agora. Tente atualizar a página.</p> : testimonials.data?.length ? <div className="space-y-3">{testimonials.data.map(item => <article key={item.id} className="rounded-xl border border-white/10 bg-black/25 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><span className="text-xs uppercase tracking-wider text-emerald-200">{labels[item.status]}</span><p className="mt-1 flex items-center gap-1 text-xs text-zinc-400"><Star className="size-3 text-emerald-300" />{item.rating ? `${item.rating}/5` : "Sem avaliação registrada"}</p><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-zinc-200">{item.content}</p></div><time className="text-xs text-zinc-500">{new Date(item.updatedAt).toLocaleString("pt-BR")}</time></div>{item.adminNote ? <div className="mt-4 border-l-2 border-emerald-300 pl-3"><span className="text-xs uppercase tracking-wider text-emerald-200">Retorno da administração</span><p className="mt-1 whitespace-pre-wrap text-sm leading-6 text-zinc-300">{item.adminNote}</p></div> : <p className="mt-4 flex items-center gap-2 text-sm text-zinc-500"><CheckCircle2 className="size-4" />Aguardando revisão da administração.</p>}</article>)}</div> : <p className="rounded-xl border border-dashed border-white/15 p-5 text-sm leading-6 text-zinc-400">Você ainda não enviou nenhum relato. Quando desejar registrar sua própria experiência, use o formulário acima.</p>}
      </section>
    </main>
  </DashboardLayout>;
}
