...
export default function ApplicationConfirmation() {
  ...
  return <main className="access-page"><div className="access-card"><a href={withAppBase("/")} className="access-back"><ChevronLeft size={15} /> Voltar para o Código Lucrativo</a><div className="access-seal"><BadgeCheck size={25} /></div><span className="office-eyebrow">Solicitação registrada</span><h1>Seu pedido de ativação foi registrado.</h1><p>O próximo passo é acompanhar o retorno com as orientações reais sobre pagamento, liberação do acesso e personalização da sua estrutura digital.</p>{code && <p className="access-note">Código para acompanhamento: <strong>{code}</strong></p>}<div className="access-steps"><div><b>01</b><span>Solicitação registrada para acompanhamento.</span></div><div><b>02</b><span>Orientação sobre o pagamento e as condições da ativação.</span></div><div><b>03</b><span>Liberação das instruções de acesso e personalização.</span></div></div><div className="access-actions"><a className="btn btn-primary" href={withAppBase(`/pedido/acompanhar?codigo=${encodeURIComponent(code ?? "")}`)}>Acompanhar solicitação <ArrowRight size={16} /></a></div></div></main>;
}
