...
import ApplicationPaymentMethods from "./ApplicationPaymentMethods";
...
export default function ApplicationConfirmation() {
  ...
  if (code && receiptSent) {
    ...
    return <main className="access-page"><div className="access-card">
      ...
      {payment.isLoading ? <p className="access-loading"><Loader2 className="inline size-4 animate-spin" /> Carregando dados do pedido...</p> : <section className="mt-5 rounded-2xl border border-emerald-300/25 bg-emerald-300/10 p-4 text-left">
        ...
        <p className="tracking-code-warning">Copie este código e salve em um local seguro. Você precisará dele para acompanhar sua ativação.</p>
  ...
  if (code) return <ApplicationPaymentMethods />;
  return <main className="access-page"><div className="access-card"><a href={withAppBase("/")} className="access-back"><ChevronLeft size={15} /> Voltar para a Página Lucrativa</a><div className="access-seal"><BadgeCheck size={25} /></div><span className="office-eyebrow">Solicitação registrada</span><h1>Seu pedido de ativação foi registrado.</h1><p>O próximo passo é acompanhar o retorno com as orientações reais sobre pagamento, liberação do acesso e personalização da sua estrutura digital.</p>{code && <><p className="access-note">Código para acompanhamento: <strong>{code}</strong></p><p className="tracking-code-warning">Copie este código e salve em um local seguro. Você precisará dele para acompanhar sua ativação.</p></>}<div className="access-steps"><div><b>01</b><span>Solicitação registrada para acompanhamento.</span></div><div><b>02</b><span>Orientação sobre o pagamento e as condições da ativação.</span></div><div><b>03</b><span>Liberação das instruções de acesso e personalização.</span></div></div><div className="access-actions"><a className="btn btn-primary" href={withAppBase(`/pedido/acompanhar?codigo=${encodeURIComponent(code ?? "")}`)}>Acompanhar solicitação <ArrowRight size={16} /></a></div></div></main>;
}
