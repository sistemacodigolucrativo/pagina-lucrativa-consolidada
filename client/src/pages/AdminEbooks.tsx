    ...
    <DashboardLayout menuItems={adminMenu} title="Administração">
      <main className="mx-auto w-full max-w-7xl space-y-7 p-5 sm:p-8">
        <header className="space-y-2">
          ...
          <h1 className="text-3xl font-semibold text-white">Academia</h1>
          <p className="max-w-3xl text-sm leading-6 text-zinc-300">
            Publique cursos em PDF e organize os materiais em sequência para os membros.
          </p>
        </header>
          ...
          <aside className="rounded-2xl border border-white/10 bg-zinc-950/60 p-3">
            ...
            <div className="max-h-[68vh] space-y-2 overflow-y-auto">
              {ebooks.isLoading ? (
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start gap-2">
                        <GraduationCap className="mt-0.5 size-4 shrink-0 text-emerald-300" />
                        <div className="min-w-0">
                          <p className="truncate text-sm font-medium text-white">{ebook.title}</p>
                          <p className="mt-1 text-xs uppercase tracking-wider text-zinc-500">
                            {ebook.status === "published" ? "Publicado" : ebook.status === "draft" ? "Rascunho" : "Arquivado"} · Curso
                          </p>
                ...
                <p className="p-3 text-sm text-zinc-400">Nenhum curso cadastrado.</p>
              )}
            </div>
          </aside>
          ...
          <div className="space-y-6">
            <form onSubmit={submit} className="space-y-4 rounded-2xl border border-white/10 bg-zinc-950/60 p-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <h2 className="font-medium text-white">{selectedId ? "Editar curso" : "Novo curso"}</h2>
                  <p className="mt-1 text-sm text-zinc-400">Envie um PDF e defina a qual curso ele pertence.</p>
                </div>
                {selectedId && (
            ...
            <section className="rounded-2xl border border-white/10 bg-zinc-950/60 p-5">
              ...
              <p className="text-sm leading-6 text-zinc-300">
                Todo PDF publicado nesta tela entra como material de curso e aparece agrupado na Academia do membro.
              </p>
            </section>
          </div>
        </section>
