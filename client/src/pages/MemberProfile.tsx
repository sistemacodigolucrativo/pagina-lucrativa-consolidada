  ...
  return <DashboardLayout menuItems={menu} title="Escritório Virtual"><main className="mx-auto w-full max-w-5xl space-y-6 p-5 sm:p-8">
    ...
    <section className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_280px]">
      <form onSubmit={handleSubmit} className="order-2 space-y-6 rounded-2xl border border-white/10 bg-zinc-950/60 p-5 lg:order-1">
        ...
        <section className="space-y-4 border-t border-white/10 pt-5"><div className="flex items-center gap-2 text-white"><MapPin size={18} className="text-emerald-300" /><h2 className="font-medium">Endereço</h2></div><p className="text-sm leading-6 text-zinc-400">O endereço principal é obrigatório para concluir a Etapa 1. Os demais campos são complementares.</p><label className="block text-sm text-zinc-200">Endereço (Rua, Avenida, etc) *<input value={form.address} onChange={event => updateField("address", event.target.value)} aria-invalid={Boolean(errors.address) || undefined} className={field} maxLength={255} />{errors.address ? <small className={errorClass} role="alert">{errors.address}</small> : null}</label><div className="grid gap-3 sm:grid-cols-2"><label className="text-sm text-zinc-200">Número <span className="text-zinc-500">(opcional)</span><input value={form.addressNumber} onChange={event => updateField("addressNumber", event.target.value)} className={field} maxLength={32} /></label><label className="text-sm text-zinc-200">Complemento <span className="text-zinc-500">(opcional)</span><input value={form.addressComplement} onChange={event => updateField("addressComplement", event.target.value)} className={field} maxLength={160} /></label><label className="text-sm text-zinc-200">CEP <span className="text-zinc-500">(opcional)</span><input value={form.postalCode} onChange={event => updateField("postalCode", event.target.value)} className={field} placeholder="00000-000" maxLength={20} /></label><label className="text-sm text-zinc-200">Bairro <span className="text-zinc-500">(opcional)</span><input value={form.district} onChange={event => updateField("district", event.target.value)} className={field} maxLength={120} /></label><label className="text-sm text-zinc-200">Cidade <span className="text-zinc-500">(opcional)</span><input value={form.city} onChange={event => updateField("city", event.target.value)} className={field} maxLength={120} /></label><label className="text-sm text-zinc-200">Estado <span className="text-zinc-500">(opcional)</span><input value={form.state} onChange={event => updateField("state", event.target.value)} className={field} maxLength={80} /></label></div></section>
