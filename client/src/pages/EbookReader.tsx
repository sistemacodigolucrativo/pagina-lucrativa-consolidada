  ...
  return (
    <DashboardLayout menuItems={menu} title="Biblioteca de execução">
      <main className="mx-auto w-full max-w-7xl space-y-7 p-5 sm:p-8">
        <header className="space-y-2"><span className="text-xs uppercase tracking-[0.16em] text-amber-300">Biblioteca de execução</span><h1 className="text-3xl font-semibold text-white">E-books e materiais de estudo</h1><p className="max-w-3xl text-sm leading-6 text-zinc-300">Consulte e-books publicados diretamente no navegador. A leitura fica restrita aos conteúdos liberados para a sua conta.</p></header>
        {ebooks.isLoading ? <div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-zinc-950/60 p-6 text-sm text-zinc-300"><LoaderCircle className="size-4 animate-spin text-amber-300" />Carregando biblioteca...</div> : ebooks.data?.length ? (
        ...
        ) : <section className="rounded-2xl border border-white/10 bg-zinc-950/60 p-8 text-center"><LibraryBig className="mx-auto size-8 text-amber-300" /><h2 className="mt-3 text-lg font-medium text-white">Nenhum e-book publicado</h2><p className="mt-2 text-sm text-zinc-400">Quando a administração liberar materiais, eles aparecerão nesta biblioteca.</p></section>}
      </main>
    </DashboardLayout>
  );
}
