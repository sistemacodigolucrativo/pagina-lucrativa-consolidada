    ...
    <DashboardLayout menuItems={menu} title="Biblioteca de execução">
      <main className="mx-auto w-full max-w-7xl space-y-7 p-5 sm:p-8">
        <header className="space-y-2">
          ...
          <h1 className="text-3xl font-semibold text-white">E-books e materiais de estudo</h1>
        ...
        {ebooks.isLoading ? (
          <div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-zinc-950/60 p-6 text-sm text-zinc-300">
          ...
          <>
            <section className="min-w-0 rounded-2xl border border-white/10 bg-zinc-950/60 p-3 sm:p-5">
              <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  <p className="text-xs font-medium uppercase tracking-wider text-zinc-500">{ebooks.data.length} materiais publicados</p>
                  <h2 className="mt-1 text-lg font-semibold text-white">Escolha um e-book para abrir o leitor.</h2>
                </div>
                <p className="text-xs leading-5 text-zinc-400 sm:text-right">O conteúdo abre em uma janela de leitura sem sair da biblioteca.</p>
              </div>
                    ...
                    >
                      <div className="flex min-w-0 gap-3">
                        ...
                        <div className="min-w-0">
                          <h3 className="text-sm font-medium text-white">{ebook.title}</h3>
                          <p className="mt-1 line-clamp-3 text-xs leading-5 text-zinc-400">{ebook.summary || "Material de estudo publicado."}</p>
            ...
            <Dialog open={readerOpen} onOpenChange={handleReaderOpenChange}>
              <DialogContent
                ...
                className="grid h-[100dvh] max-h-[100dvh] w-screen max-w-none grid-rows-[auto_minmax(0,1fr)] gap-0 overflow-hidden rounded-none border-white/10 bg-zinc-950 p-0 text-white shadow-2xl sm:h-[min(92dvh,920px)] sm:max-h-[92dvh] sm:w-[min(1120px,calc(100vw-2rem))] sm:max-w-[min(1120px,calc(100vw-2rem))] sm:rounded-2xl"
              >
                <DialogHeader className="border-b border-white/10 px-4 pb-3 pt-[calc(env(safe-area-inset-top)+1rem)] text-left sm:px-5 sm:pt-5">
                  <div className="flex min-w-0 items-start justify-between gap-4">
                    <div className="min-w-0">
                      <span className="text-xs font-semibold uppercase tracking-[0.16em] text-emerald-300">Leitura selecionada</span>
                      <DialogTitle className="mt-1 truncate text-lg font-semibold text-white sm:text-xl">{readerTitle}</DialogTitle>
                      <DialogDescription className="mt-1 line-clamp-2 text-xs leading-5 text-zinc-400 sm:text-sm">
                        {readerSummary}
                      </DialogDescription>
                    </div>
                    <DialogClose asChild>
                ...
                <div className="min-h-0 overflow-hidden px-2 pb-[calc(env(safe-area-inset-bottom)+0.5rem)] pt-2 sm:p-4">
                  {selected.isLoading ? (
                    <div className="flex h-full min-h-[55dvh] items-center justify-center gap-2 rounded-xl border border-white/10 bg-black/25 text-sm text-zinc-400">
                      <LoaderCircle className="size-4 animate-spin text-emerald-300" />
                      Abrindo e-book...
                    </div>
                  ) : selected.isError ? (
                    <div className="flex h-full min-h-[55dvh] items-center justify-center rounded-xl border border-white/10 bg-black/25 p-6 text-center text-sm text-zinc-400">
          ...
          <section className="rounded-2xl border border-white/10 bg-zinc-950/60 p-8 text-center">
            <LibraryBig className="mx-auto size-8 text-emerald-300" />
            <h2 className="mt-3 text-lg font-medium text-white">Nenhum e-book publicado</h2>
            <p className="mt-2 text-sm text-zinc-400">Quando a administração liberar materiais, eles aparecerão nesta biblioteca.</p>
          </section>
        )}
      </main>
    </DashboardLayout>
