...
const moduleDetails: Record<string, { eyebrow: string; title: string; detail: string; notes: string[] }> = {
  ...
  "/membros/blog": { eyebrow: "Biblioteca de Recursos", title: "Biblioteca de Recursos", detail: "Acesse materiais prontos para apoiar a divulgação do Código Lucrativo.", notes: ["Use banners, textos, descrições e conteúdos promocionais liberados pela administração.", "Todos os recursos ficam centralizados nesta biblioteca."] },
  ...
  "/membros/artigos": { eyebrow: "Biblioteca de Recursos", title: "Biblioteca de Recursos", detail: "Encontre imagens, banners, textos, copies e outros conteúdos preparados para suas divulgações.", notes: ["Use esta área para conteúdos promocionais prontos.", "Ferramentas, automações e links externos ficam centralizados na Biblioteca de Recursos."] },
...
export default function MemberOffice() {
  ...
  const renderBody = () => {
    ...
    if (currentPath === "/membros/academia") return <><SectionIntro eyebrow="Academia de execução" title="Aprenda e aplique" detail="Cursos publicados e progresso individual para transformar estudo em ações da sua operação digital." />{academy.data?.length ? <div className="office-course-grid">{academy.data.map(course => <article key={course.id}><span>{course.level}</span><h2>{course.title}</h2><p>{course.summary || "Conteúdo em preparação."}</p><footer><small>{"ebookCount" in course && course.ebookCount ? `${course.ebookCount} materiais` : `${course.durationMinutes} min`}</small><a href={withAppBase(`/membros/curso/${course.routeKey}`)}>Abrir curso</a></footer></article>)}</div> : <section className="office-empty"><span className="office-empty-mark">PL</span><h2>Nenhum curso disponível no momento.</h2><p>Os conteúdos publicados pela administração aparecerão aqui, organizados por etapa e tema.</p></section>}</>;
