...
import { ArrowLeft, ArrowRight, Clipboard, CreditCard, Loader2, QrCode } from "lucide-react";
      ...
      <section className="grid gap-5 lg:grid-cols-[1fr_18rem]">
        <div className="space-y-4">
          {pixKey ? <article className="rounded-2xl border border-emerald-300/25 bg-emerald-300/10 p-5">
            ...
            <a href={`${instructionsUrl}?metodo=pix`} className="mt-4 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-lg bg-emerald-300 px-4 py-2 text-sm font-semibold text-black sm:w-auto">Ver dados PIX <ArrowRight className="size-4" /></a>
          </article> : null}
          ...
          {paymentLinks.map(link => <article key={link.id} className="rounded-2xl border border-white/10 bg-zinc-950/70 p-5">
            ...
            <div className="mt-4 flex flex-col gap-3 sm:flex-row">
              ...
              <a href={`${instructionsUrl}?metodo=${encodeURIComponent(link.label)}`} className="inline-flex min-h-11 items-center justify-center rounded-lg border border-white/15 px-4 py-2 text-sm font-semibold text-white hover:bg-white/5">Já paguei / enviar comprovante</a>
            </div>
          </article>)}
